...
Clazz.load (["JU.P3", "$.SB"], "J.adapter.smarter.XtalSymmetry", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.BS", "$.Lst", "$.M3", "$.M4", "$.P3i", "$.PT", "$.V3", "J.adapter.smarter.Atom", "JS.Symmetry", "$.SymmetryOperation", "JU.BSUtil", "$.SimpleUnitCell"], function () {
...
if (this.doPackUnitCell && this.applySymmetryToBonds) {
var atoms = this.asc.atoms;
var bonds = this.asc.bonds;
for (var i = this.asc.bondCount; --i >= 0; ) {
bonds[i].distance = atoms[bonds[i].atomIndex1].distance (atoms[bonds[i].atomIndex2]);
}
...
this.ndims = Clazz.floatToInt (this.symmetry.getUnitCellInfoType (6));
...
if (this.acr.fixJavaFloat) JU.PT.fixPtFloats (ptAtom, 100000.0);
if (!this.isWithinCell (this.ndims, ptAtom, this.minXYZ0.x, this.maxXYZ0.x, this.minXYZ0.y, this.maxXYZ0.y, this.minXYZ0.z, this.maxXYZ0.z, this.packingError)) continue;
...
this.asc.addNewBondWithOrder (ia1, ia2, bond.order).distance = d;
}}
}}
return pt;
